...
from tools.tompy import signal_stats, sample_rate_check, small
from tools.tompy import enter_float
from tools.tompy import GetInteger2
from tools.tompy import time_history_plot
from tools.tompy import WriteData2
from tools.Time2PSD import psdftt
from tools.acc2rdf import digitize

from tools.signal_utilities import EnterPSD

from tools.velox_correction import velox_correction
...
def get_acc_from_psd(M, s_max, s_zero, sr, signalDuration):
    ...
    plt.plot(freq_spec, amp_spec, label="specification", linewidth=1.0)
    title_string = 'Preliminary Power Spectral Density   ' + str("%6.3g" % psd_grms_test) + ' GRMS Overall '
    plt.title(title_string)
    plt.xlabel(' Frequency (Hz) ')
    plt.grid(True)
    plt.xscale('log')
    plt.yscale('log')
    plt.ylim([1e-4, max(psd_test) * 1.1])
    ...
    print(" ")
    print("Enter the acceleration time history filename [only name]")
    ...
    file_path = "digitized/" + a_output_file_path.rstrip('\n')
    WriteData2(np, TT, psd_th, file_path)

    digitize(psd_th, 2, file_path  + ".rdf", s_max, s_zero)
    ...
    time_history_plot(TT, psd_th, 3, 'Time(sec)', 'Accel(G)', title_string, 'img/accel_' + a_output_file_path[:-1])
    
    sd = std(velox)
    string_value1 = "  %6.3g" % sd
    
    title_string = 'Velocity' + string_value1 + ' in/sec RMS'
    time_history_plot(TT, velox, 4, 'Time(sec)', 'Vel(in/sec)', title_string, 'img/velox_' + a_output_file_path[:-1])
    
    sd = std(dispx)
    string_value1 = "  %6.3g" % sd
    
    title_string = 'Displacement' + string_value1 + ' inch RMS'
    time_history_plot(TT, dispx, 5, 'Time(sec)', 'Disp(inch)', title_string, 'img/disp_' + a_output_file_path[:-1])
    
    ############################################################################
    
    plt.figure(6)
    plt.hist(psd_th_store, 400)
    plt.title('Acceleration Histogram')
    plt.xlabel(' Accel (G) ')
    plt.ylabel(' Counts ')
    plt.grid()
    plt.savefig('img/hits_' + a_output_file_path[:-1])
    ...
    plt.figure(7)
    plt.plot(freq, full, label="synthesis")
    plt.plot(freq_spec, amp_spec, label="specification")
    title_string = 'Power Spectral Density   ' + str("%6.3g" % psd_rms) + ' GRMS Overall '
    plt.title(title_string)
    plt.xlim([xmin, xmax])
    plt.ylim([ymin, ymax])
